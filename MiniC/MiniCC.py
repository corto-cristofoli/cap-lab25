...
def main(inputname, reg_alloc, mode,
    ...
    with open(output_name, 'w') if output_name else sys.stdout as output:
        ...
        for function in visitor3.get_functions():
            ...
            if debug_graphs:
                s = f"{basename}.{code.fdata.get_name()}.dot"
                print("CFG:", s)
            ...
            if mode.is_ssa():
                ...
                if ssa_graphs:
                    s = f"{basename}.{code.fdata.get_name()}.enterssa.dot"
                    print("SSA:", s)
                    code.print_dot(s, view=True)
                if mode == Mode.OPTIM:
                    ...
                    if ssa_graphs:
                        s = f"{basename}.{code.fdata.get_name()}.optimssa.dot"
            ...
            if reg_alloc == "naive":
                ...
                if mode.is_ssa():
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"Invalid dataflow form: liveness file not found for {form}.")
                else:
                    try:
            ...
            if mode.is_ssa() and ssa_graphs:
                s = f"{basename}.{code.fdata.get_name()}.exitssa.dot"
